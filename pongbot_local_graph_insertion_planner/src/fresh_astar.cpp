...
{
  if (isBlocked(grid, cell, options)) {
    return kInfinity;
  }
  const auto cost = grid.costs[cell];
  ...
  const GridSnapshot & grid, std::size_t cell, const SearchOptions & options)
{
  ...
  if (!grid.validIndex(cell) || isBlocked(grid, cell, options)) {
    return result;
  }

  const auto current_x = grid.x(cell);
  const auto current_y = grid.y(cell);
  for (int dy = -1; dy <= 1; ++dy) {
    for (int dx = -1; dx <= 1; ++dx) {
      ...
      const auto neighbor_x = static_cast<long long>(current_x) + dx;
      const auto neighbor_y = static_cast<long long>(current_y) + dy;
      const bool inside_grid =
        neighbor_x >= 0 &&
        neighbor_y >= 0 &&
        neighbor_x < static_cast<long long>(grid.size_x) &&
        neighbor_y < static_cast<long long>(grid.size_y);
      if (!inside_grid) {
        continue;
      }

      const auto next = grid.index(
        static_cast<std::size_t>(neighbor_x),
        static_cast<std::size_t>(neighbor_y));
      if (isBlocked(grid, next, options)) {
        continue;
      }

      const bool diagonal = dx != 0 && dy != 0;
      if (diagonal) {
        const auto side_x = grid.index(
          static_cast<std::size_t>(static_cast<long long>(current_x) + dx),
          current_y);
        const auto side_y = grid.index(
          current_x,
          static_cast<std::size_t>(static_cast<long long>(current_y) + dy));
        if (isBlocked(grid, side_x, options) || isBlocked(grid, side_y, options)) {
          continue;
        }
      }
...
double edgeCost(
  ...
  const SearchOptions & options)
{
  if (!grid.validIndex(from) || !grid.validIndex(to)) {
    return kInfinity;
  }
  ...
  const auto dx = grid.x(from) > grid.x(to) ?
    grid.x(from) - grid.x(to) : grid.x(to) - grid.x(from);
  const auto dy = grid.y(from) > grid.y(to) ?
    grid.y(from) - grid.y(to) : grid.y(to) - grid.y(from);
  const bool diagonal = dx == 1 && dy == 1;
  const double metric = (diagonal ? kSqrt2 : 1.0) * grid.resolution;
...
double heuristic(const GridSnapshot & grid, std::size_t from, std::size_t to)
{
  const double dx = static_cast<double>(
    grid.x(from) > grid.x(to) ?
    grid.x(from) - grid.x(to) : grid.x(to) - grid.x(from));
  const double dy = static_cast<double>(
    grid.y(from) > grid.y(to) ?
    grid.y(from) - grid.y(to) : grid.y(to) - grid.y(from));
  ...
  const SearchOptions & options, double * cost)
{
  ...
  for (std::size_t i = 0; i < path.size(); ++i) {
    if (!grid.validIndex(path[i]) || isBlocked(grid, path[i], options)) {
      return false;
    }
    if (i > 0) {
  ...
  const SearchOptions & options)
{
  ...
  if (invalid_input) {
    result.status = SearchStatus::kInvalidInput;
    result.reason = "invalid grid, options, or endpoint";
    return result;
  }
  ...
  g[start] = 0.0;
  open.push({heuristic(grid, start, goal), 0.0, start});
    ...
    if (current.cell == goal) {
      ...
      for (auto node = goal; node != start; node = parent[node]) {
        if (node >= parent.size() || parent[node] >= parent.size()) {
          result.status = SearchStatus::kNoPath;
          result.reason = "parent invariant failure";
          return result;
        }
  ...
  result.status = SearchStatus::kNoPath;
  result.reason = "open set exhausted";
  return result;
}
