...
std::vector<TerrainPoint> TrackingPathRefiner::resampleGrid(
  const std::vector<TerrainPoint> & path, const GridPathEvaluator & evaluator,
  bool & valid) const
{
  valid = false;
  if (path.empty()) {return {};}
  ...
  for (std::size_t index = 1U; index < path.size(); ++index) {
    ...
    for (std::size_t step = 1U; step <= intervals; ++step) {
      ...
      const Point2D point{from.x + ratio * (to.x - from.x),
        from.y + ratio * (to.y - from.y)};
      const auto node = evaluator.evaluatePoint(point);
      if (!node.valid) {return path;}
      output.push_back({point.x, point.y, node.elevation_m});
    }
  }
  valid = evaluator.evaluatePolyline(output).valid;
  return valid ? output : path;
}

TrackingPathRefinerResult TrackingPathRefiner::refineGrid(
  const std::vector<TerrainPoint> & raw_path, const GridPathEvaluator & evaluator) const
{
  TrackingPathRefinerResult result;
  result.raw_point_count = raw_path.size();
  result.raw_max_heading_change_rad = maximumHeadingChange(raw_path);
  const auto raw_evaluation = evaluator.evaluatePolyline(raw_path);
  result.raw_cost = raw_evaluation.total_cost;
  result.path = raw_path;
  if (!raw_evaluation.valid) {
    result.used_raw_fallback = true;
  ...
  if (!parameters_.enabled) {
    result.success = true;
    result.tracking_point_count = raw_path.size();
    result.tracking_cost = raw_evaluation.total_cost;
    result.tracking_max_heading_change_rad = result.raw_max_heading_change_rad;
    return result;
  }
  ...
  if (selected.size() >= 3U) {
    ...
    for (std::size_t pass = 0U; pass < parameters_.smoothing_passes; ++pass) {
      for (std::size_t index = 1U; index + 1U < working.size(); ++index) {
        ...
        const Point2D candidate{
          (working[index - 1U].x + working[index].x + working[index + 1U].x) / 3.0,
          (working[index - 1U].y + working[index].y + working[index + 1U].y) / 3.0};
        const auto node = evaluator.evaluatePoint(candidate);
        const bool accepted = node.valid && evaluator.evaluateSegment(
          {working[index - 1U].x, working[index - 1U].y}, candidate).valid &&
          evaluator.evaluateSegment(
          candidate, {working[index + 1U].x, working[index + 1U].y}).valid;
        if (accepted) {
          working[index] = {candidate.x, candidate.y, node.elevation_m};
          ++result.smoothing_accepts;
    ...
    const auto smoothed = evaluator.evaluatePolyline(working);
    if (smoothed.valid && smoothed.total_cost <=
      raw_evaluation.total_cost * (1.0 + parameters_.max_cost_increase_ratio) + kTolerance)
    {
      selected = std::move(working);
  ...
  bool resampled_valid = false;
  auto resampled = resampleGrid(selected, evaluator, resampled_valid);
  if (resampled_valid) {selected = std::move(resampled);}
  const auto final_evaluation = evaluator.evaluatePolyline(selected);
  if (!final_evaluation.valid) {
    selected = raw_path;
    result.used_raw_fallback = true;
  }
  const auto selected_evaluation = evaluator.evaluatePolyline(selected);
  if (!selected_evaluation.valid) {
    result.path.clear();
    result.tracking_point_count = 0U;
    return result;
  }
  result.path = std::move(selected);
  result.tracking_point_count = result.path.size();
  result.tracking_cost = selected_evaluation.total_cost;
  result.tracking_max_heading_change_rad = maximumHeadingChange(result.path);
  result.success = true;
  return result;
}
