...
public:
  ...
  : Node("joy_to_lcm")
  {
    ...
    joy_topic_   = declare_parameter<std::string>("joy_topic", "/joy_wireless");
...
private:
  ...
  void joyCb(const sensor_msgs::msg::Joy::SharedPtr msg)
  {
    ...
    // 기존 buttons_[3] = Navigation Mode ON/OFF
    last_buttons_[3] = getButton(msg->buttons, 1);       // RUBI
    ...
    if (debug_raw_joy_) {
      RCLCPP_INFO_THROTTLE(
        ...
        "JOY remap | WALKREADY=%d POLICYON=%d NAVIGATION=%d TORQUEOFF=%d, linear_x=%.2f, linear_y=%.2f, ang_z=%.2f",
        last_buttons_[10], 
        last_buttons_[8], 
        last_buttons_[3], 
        last_buttons_[9], 
